...
int main(int argc, char *argv[]) {
	...
	pthread_t thread[MAX_NB_THREAD];
	ssh_session session[MAX_NB_THREAD];
	int useThread = 0;
	...
	while (1) {
		session[useThread]=ssh_new();

		if (ssh_bind_accept(sshbind, session[useThread]) == SSH_ERROR) {
		...
		// Create thread
		if (pthread_create(&thread[useThread], 0, (void *)handle_auth, session[useThread]))
			break;

		// Update the structure of threads
		i = 0;
		while (i < MAX_NB_THREAD) {
			if (thread[i] != 0 && pthread_tryjoin_np(thread[i], NULL) != EBUSY) {
				if (DEBUG) { printf("Liberation du thread %d\n", i); }
				thread[i] = 0;
